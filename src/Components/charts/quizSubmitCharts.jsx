...
export const QuizSubmissionsChartContent = ({
  submissions,
  loading,
  error,
  stats,
...
const QuizSubmissionsChart = () => {
  ...
  const stats =
    submissions.length > 0
      ? {
          ...
          avgAccuracy: (
            submissions.reduce((sum, s) => sum + s.accuracy, 0) /
            submissions.length
          ).toFixed(2),
        }
      : null;
  ...
  const passFail = [
    {
      name: "Pass",
      value: submissions.filter((s) => s.score >= passScore).length,
    },
    {
      name: "Fail",
      value: submissions.filter((s) => s.score < passScore).length,
      ...
      {stats && (
        <Box mb={2}>
          <Typography variant="subtitle1">
            ...
            Average Score: <strong>{stats.avgScore}</strong> | Average Accuracy:{" "}
            <strong>{stats.avgAccuracy}%</strong>
          </Typography>
        </Box>
      )}

      <QuizSubmissionsChartContent
